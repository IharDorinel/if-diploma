...
import { incrementGoods, decrementGoods } from './actionCreators';
import {increaseCount, decreaseCount} from "./store/reducers/cartReducer";
import {Link} from "react-router-dom";

const CartBagItem = ({item, count, handleClick, selectHandleClick, targetValue}) => {

  const countGoods = count.filter(elem => elem.id === item.id)
console.log(countGoods[0])
